...
class DB_entity(object):
    ...
    class __Singleton:
        ...
        def getAnnounceAttributed(self):
            with self.conn.cursor(oursql.DictCursor) as cursor:
                query = '''SELECT * FROM announces WHERE id IN (SELECT announce FROM assignments) LIMIT 0,50'''
                cursor.execute(query)
                return cursor.fetchall()
            
        def getAnnounceUnattributed(self):
            with self.conn.cursor(oursql.DictCursor) as cursor:
                query = '''SELECT * FROM announces WHERE id NOT IN (SELECT announce FROM assignments) LIMIT 0,50'''
                cursor.execute(query)
                return cursor.fetchall()
